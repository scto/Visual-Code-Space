 ...
 *
 * This class provides a way to embed the Monaco Editor within an Android application, allowing for
 * rich code editing capabilities. It uses a WebView to display the editor and communicates with it
 * using JavaScript.
 *
...
class MonacoEditor @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) :
    ...
    private fun loadJs(script: String, resultCallback: (String) -> Unit = {}) {
        editorScope.launch {
            withContext(Dispatchers.Unconfined) { evaluateJavascript(script, resultCallback) }
        }
    }
    ...
    val canUndo
        get() = webInterface.canUndo

    val canRedo
        get() = webInterface.canRedo
    ...
    fun selectText(range: Range) {
        loadJs(
            "selectText(${range.startLineNumber}, ${range.startColumn}, ${range.endLineNumber}, ${range.endColumn});"
        )
    }
