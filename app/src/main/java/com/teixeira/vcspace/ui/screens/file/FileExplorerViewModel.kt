...
class FileExplorerViewModel : ViewModel() {
    ...
    val openedFolder
        get() = _openedFolder.asStateFlow()

    private val _isGitRepo = MutableStateFlow(false)
    val isGitRepo
        get() = _isGitRepo.asStateFlow()
    ...
    private fun updateGitRepoStatus(file: File) {
        _isGitRepo.update {
            file.asRawFile()?.let { jfile ->
                GitManager.isGitRepository(jfile).also {
                    if (it) GitManager.instance.initialize(jfile)
                }
            } ?: false
        }
    }
    ...
    fun checkIfGitRepo() {
        _openedFolder.value?.let { file -> updateGitRepoStatus(file) }
    }
