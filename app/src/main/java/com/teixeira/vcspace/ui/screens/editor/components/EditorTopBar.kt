...
@Composable
fun EditorTopBar(modifier: Modifier = Modifier, editorViewModel: EditorViewModel) {
    ...
    val areModifiedFiles by
        remember(selectedFileIndex) { derivedStateOf { editors.any { it.value.modified } } }
    ...
    var isKeyboardOpen by remember {
        mutableStateOf(KeyboardUtils.isSoftInputVisible(context as Activity))
    }
    ...
    LifecycleStartEffect(key1 = lifecycleOwner) {
        ...
        onStopOrDispose { KeyboardUtils.unregisterSoftInputChangedListener(context.window) }
    }
    ...
    VCSpaceTopBar(
        ...
        actions = {
            AnimatedVisibility(visible = isFileRunnable(selectedFile?.file)) {
                    ...
                    IconButton(
                        onClick = {
                            when (selectedFile?.file?.extension) {
                                ...
                                "htm" -> {
                                    selectedFile.file.parent?.let { directory ->
                                        ...
                                        runCatching {
                                                ...
                                                ToastUtils.showLong(
                                                    "Server started on http://localhost:$assignedPort"
                                                )
                                                ...
                                                customTabs.launchUrl(
                                                    context,
                                                    "http://localhost:$assignedPort/${selectedFile.file.name ?: ""}"
                                                        .toUri(),
                                                )
                                            }
                                            .onFailure {
                                ...
                                "md" -> {
                                    context.startActivity(
                                        Intent(context, MarkdownPreviewActivity::class.java).apply {
                                            putExtra(
            ...
            LaunchedEffect(Unit) {
                commandPaletteManager.addCommand(
                    newCommand("Terminal", "Ctrl+T") { context.open(TerminalActivity::class.java) },
                    newCommand("Search", "Ctrl+K") { selectedEditor?.beginSearchMode() },
                )
            }
            ...
            Box {
                ...
                DropdownMenu(
                    ...
                    DropdownMenuItem(
                        ...
                        leadingIcon = {
                            Icon(Icons.Rounded.KeyboardCommandKey, contentDescription = null)
                        },
                        trailingIcon = { Text("Ctrl+Shift+P") },
                    )
                    ...
                    menuManager.menuItems.forEach { menu ->
                        AnimatedVisibility(menu.visible) {
                            DropdownMenuItem(
                                ...
                                leadingIcon =
                                    menu.icon?.let { { Icon(it, contentDescription = null) } },
                                enabled = menu.enabled,
                                trailingIcon =
                                    menu.shortcut?.let { { Text(it) } }
...
fun FileMenu(
    ...
    val launchFileCreate = remember { { createFile.launch("filename.txt") } }
    ...
    val saveFile = remember { { scope.launch { editorViewModel.saveFile() } } }

    val saveAll = remember { { scope.launch { editorViewModel.saveAll() } } }
    ...
    DropdownMenu(
        ...
        menuManager.fileMenuItems.forEach { menu ->
            ...
            DropdownMenuItem(
                text = { Text(menu.title) },
                leadingIcon = menu.icon?.let { { Icon(it, contentDescription = null) } },
                trailingIcon = menu.shortcut?.let { { Text(it) } },
                enabled = menu.enabled,
                onClick = {
                    menu.onClick()
...
private fun extractPythonFile(
    ...
    if (pythonDownloaded) {
        ...
        scope.launchWithProgressDialog(
            ...
            configureBuilder = {
                it.setMessage(strings.python_extracting_python_compiler).setCancelable(false)
            },
            ...
            JFile(filePath).inputStream().use { temp7zStream ->
                ...
                val exitCode =
                    P7ZipApi.executeCommand(
                        "7z x ${file.absolutePath} -o${context.filesDir.absolutePath}"
                    )
                ...
                object : OnDownloadListener {
                    ...
                    override fun onError(error: Error) {
                        ...
                        Snackbar.make(
                                ...
                                else if (error.isServerError) "Server error!"
                                else "Download failed! Something went wrong.",
