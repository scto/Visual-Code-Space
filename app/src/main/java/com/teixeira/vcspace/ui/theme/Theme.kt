...
fun VCSpaceTheme(
    ...
    val darkTheme by
        remember(followSystemTheme, systemTheme, isDarkMode) {
            mutableStateOf(if (followSystemTheme) systemTheme else isDarkMode)
        }

    val colorScheme =
        remember(darkTheme, isAmoledMode, isDynamicColor) {
...
@Composable
fun VCSpaceTerminalTheme(content: @Composable () -> Unit) {
...
private fun ColorScheme.maybeAmoled(isAmoledMode: Boolean): ColorScheme {
    return if (isAmoledMode) {
        copy(surface = Color.Black, inverseSurface = Color.White, background = Color.Black)
    } else this
}
