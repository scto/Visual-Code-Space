...
class EditorViewModel : ViewModel() {
    data class OpenedFile(val file: File, val isModified: Boolean = false)
    ...
    data class UiState(
        ...
        val selectedFile
            get() = openedFiles.getOrNull(selectedFileIndex)
    }
    ...
    fun getEditorForFile(context: Context, file: File, isAdvancedEditor: Boolean = false): View {
        return if (isAdvancedEditor) {
            ...
            _editors.getOrPut(file.path) { CodeEditorView(context, file) }
        }
    }
    ...
    fun rememberLastFiles() {
        val lastOpenedFiles =
            Gson()
                .toJson(
                    FileHistory(
                        uiState.value.openedFiles.mapNotNull {
    ...
    fun setModified(file: File, modified: Boolean) {
        _uiState.update { currentState ->
            val updatedFiles =
                currentState.openedFiles.map { openedFile ->
    ...
    fun addFile(file: File) {
        runCatching {
                ...
                newOpenedFiles.remove(
                    newOpenedFiles.find { it.file.name == "untitled.txt" && !it.isModified }
                )
    ...
    fun addFiles(files: List<File>) {
        viewModelScope.launch { files.forEach { addFile(it) } }
    }
    ...
    fun closeOthers(index: Int) {
        ...
        _uiState.value = uiState.value.copy(openedFiles = newOpenedFiles, selectedFileIndex = 0)
    }
