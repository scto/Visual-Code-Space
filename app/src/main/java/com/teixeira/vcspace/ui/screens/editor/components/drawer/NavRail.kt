...
@Composable
fun NavRail(modifier: Modifier = Modifier, selectedItemIndex: Int) {
    val navigationRailItems =
        listOf(
    ...
    val navRailItemIconsUnselected =
        listOf(
    ...
    NavigationRail(modifier = modifier.widthIn(max = 60.dp)) {
        navigationRailItems.fastForEachIndexed { index, name ->
            NavigationRailItem(
                icon = {
                    Icon(
                        imageVector =
                            if (selectedItemIndex == index) navRailItemIconsSelected[index]
                            else navRailItemIconsUnselected[index],
