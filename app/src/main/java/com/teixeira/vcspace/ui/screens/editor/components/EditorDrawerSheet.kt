...
fun EditorDrawerSheet(
    ...
    Row(modifier = Modifier.fillMaxSize()) {
        ...
        Column {
            Heading(title = title, onCloseDrawerRequest = { closeDrawer() })
...
fun NavigationSpaceActions(
    ...
    LaunchedEffect(Unit) {
        navigationSpaceState.apply {
            add(NavigationSpaceItem(id = 99, icon = Icons.Rounded.Close, title = close))
            add(NavigationSpaceItem(id = 0, icon = Icons.Rounded.Refresh, title = refresh))
            add(NavigationSpaceItem(id = 1, icon = Icons.Rounded.Add, title = add))
        }
    }

    NavigationSpace(modifier = modifier, state = navigationSpaceState, onItemClick = onItemClick)
}
