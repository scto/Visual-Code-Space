...
fun EditorScreen(
    ...
    LaunchedEffect(selectedFileIndex) { viewModel.rememberLastFiles() }
    ...
    DisposableEffect(openLastFiles) {
        ...
        onDispose { viewModel.rememberLastFiles() }
    }
    ...
    codeExplanationResponse?.let {
        CodeExplanationSheet(response = it, onDismissRequest = { codeExplanationResponse = null })
    }

    importComponentResponse?.let {
        ImportComponentsSheet(response = it, onDismissRequest = { importComponentResponse = null })
    }
                ...
                if (editorView is MonacoEditor) {
                    AnimatedVisibility(visible = true) {
                        AndroidView(
                            ...
                            modifier = Modifier.fillMaxWidth().fillMaxHeight(fraction = 0.92f),
                        )
                    }
        ...
        editorView.addOnEditorLoadCallback {
            ...
            editorView.apply {
                ...
                setAcceptSuggestionOnEnter(
                    AcceptSuggestionOnEnter.fromValue(acceptSuggestionOnEnter)
                )
                setFolding(folding)
                ...
                inlineCompletionProvider =
                    InlineCompletionProvider { language, textBeforeCursor, textAfterCursor ->
                        language
                    }
        ...
        editorView.apply {
            if (file.exists()) {
                ...
                scope.launch(Dispatchers.IO) {
                    ...
                    withContext(Dispatchers.Main) { text = contents }
                }
            } else {
                text = ""
            }
    ...
    ConfigureEditor(
        ...
        onExplainCodeListener = { code ->
            scope.launchWithProgressDialog(
                ...
                Gemini.explainCode(code.toString()).onSuccess(onExplainCodeResponse).onFailure {
                    scope.launch {
...
private fun ConfigureEditor(
    ...
    val items = remember {
        mutableStateListOf<EditorTextActionItem>().apply { addAll(actionItems) }
    }
    val editorTextActionWindowExpandThreshold by rememberEditorTextActionWindowExpandThreshold()

    val editorTextActionWindow =
        editorTextActionWindow(
            items = items,
                ...
                .setTheme(
                    when (colorScheme) {
                        context.getString(R.string.pref_editor_colorscheme_value_followui) ->
                            if ((followSystemTheme && isDarkTheme) || isDarkMode) "darcula"
                            else "quietlight"
