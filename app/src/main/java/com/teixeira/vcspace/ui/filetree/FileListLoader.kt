...
class FileListLoader(private val cacheFiles: MutableMap<String, CacheEntry> = mutableMapOf()) {
    ...
    suspend fun loadFileList(
        ...
        withContext(Dispatchers.Main) {
            when (val value = cacheFiles[path.absolutePath]) {
                null -> {
                    val deferred = async(Dispatchers.IO) { getFileList(path).toMutableList() }
    ...
    object FileListLoaderSaver : Saver<FileListLoader, Map<String, List<JFile>>> {
        override fun restore(value: Map<String, List<JFile>>): FileListLoader? {
            return FileListLoader(
                mutableMapOf<String, CacheEntry>().also { cache ->
                    value.mapValuesTo(cache) { entry ->
                        Loaded(
                            mutableListOf<File>().also { list ->
