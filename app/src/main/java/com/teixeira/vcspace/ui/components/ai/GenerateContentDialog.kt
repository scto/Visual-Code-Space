...
fun GenerateContentDialog(
    ...
    if (showDialog) {
        AlertDialog(
            ...
            title = { Text(text = stringResource(strings.generate_code)) },
            text = {
                OutlinedTextField(
                    value = prompt,
                    ...
                    onClick = {
                        if (prompt.isNotEmpty()) {
                            scope.launchWithProgressDialog(
                                ...
                                Gemini.generateCode(prompt = prompt, fileExtension = fileExtension)
                                    .onSuccess { response ->
                                        val text =
                                            response.candidates
                                                .first()
                                        ...
                                        withContext(Dispatchers.Main) {
                                            if (editor is MonacoEditor) {
                                                ...
                                                editor.insert(
                                                    text =
                                                        Gemini.removeBackticksFromMarkdownCodeBlock(
                                                            text
                                                        ),
                                                    position = position,
                                                )
