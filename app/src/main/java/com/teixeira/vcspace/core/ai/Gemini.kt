...
object Gemini {
    ...
    private suspend fun generateContent(prompt: String) =
        withContext(Dispatchers.IO) {
            runCatching { model.generateContent(content { text(prompt) }) }
        }
        ...
        if (!isJetpackComposeCode(code)) {
            return Result.failure(
                IllegalArgumentException(
                    "The provided code does not appear to be Jetpack Compose code."
                )
            )
        }
    ...
    suspend fun generateCode(
        ...
        return generateContent(
            "Write the code on based on my prompt${if (!fileExtension.isNullOrEmpty()) " for file extension $fileExtension" else ""} and provide me only code:\nThe prompt:\n\n$prompt"
        )
    }
