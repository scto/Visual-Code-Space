...
@Composable
private fun <T> rememberPreference(key: Preferences.Key<T>, defaultValue: T): MutableState<T> {
    val coroutineScope = rememberCoroutineScope()
    val context = LocalContext.current

    val state by
        remember { context.dataStore.data.map { it[key] ?: defaultValue } }
...
object Settings {
    ...
    object General {
        ...
        @Composable
        fun rememberFollowSystemTheme() =
            rememberPreference(key = FOLLOW_SYSTEM_THEME, defaultValue = true)
        ...
        @Composable
        fun rememberEnableGestureInDrawer() =
            rememberPreference(key = ENABLE_GESTURE_IN_DRAWER, defaultValue = true)
    }
    ...
    object File {
        ...
        @Composable
        fun rememberShowHiddenFiles() =
            rememberPreference(key = SHOW_HIDDEN_FILES, defaultValue = false)

        @Composable
        fun rememberLastOpenedFile() =
            rememberPreference(key = REMEMBER_LAST_OPENED_FILE, defaultValue = false)
    }
    ...
    object Editor {
        ...
        @Composable fun rememberFontSize() = rememberPreference(key = FONT_SIZE, defaultValue = 14f)
        ...
        @Composable fun rememberUseTab() = rememberPreference(key = USE_TAB, defaultValue = true)

        @Composable
        fun rememberDeleteLineOnBackspace() =
            rememberPreference(key = DELETE_LINE_ON_BACKSPACE, defaultValue = true)

        @Composable
        fun rememberDeleteIndentOnBackspace() =
            rememberPreference(key = DELETE_INDENT_ON_BACKSPACE, defaultValue = false)

        @Composable
        fun rememberEditorTextActionWindowExpandThreshold() =
            rememberPreference(key = EDITOR_TEXT_ACTION_WINDOW_EXPAND_THRESHOLD, defaultValue = 10)
    }
    ...
    object Monaco {
        ...
        @Composable fun rememberFontSize() = rememberPreference(key = FONT_SIZE, defaultValue = 14)
        ...
        @Composable
        fun rememberAcceptSuggestionOnEnter() =
            rememberPreference(
        ...
        @Composable
        fun rememberCursorBlinkingStyle() =
            rememberPreference(
                key = CURSOR_BLINKING_STYLE,
                defaultValue = TextEditorCursorBlinkingStyle.Phase.value,
            )

        @Composable fun rememberFolding() = rememberPreference(key = FOLDING, defaultValue = true)
