...
class PluginContextImpl(
    ...
    override fun registerCommand(command: EditorCommand) {
        ...
        commandManager.addCommand(
            Command.newCommand(command.name, command.keyBinding) { command.execute(editor) }
        )
    }
    ...
    override fun <T : View> createViewPanel(
        ...
        return createComposePanelInternal(
            title,
            @Composable {
                AndroidView(
    ...
    override fun doActionOnIOThread(action: Runnable) {
        activity.lifecycleScope.launch(Dispatchers.IO + SupervisorJob()) { action.run() }
    }
    ...
    private fun createComposePanelInternal(
        ...
        fun PanelComposable() {
            ...
            LaunchedEffect(panel.offset) { internalOffset = panel.offset }
