...
object Editor {
    ...
    val LocalCommandPaletteManager = staticCompositionLocalOf { CommandPaletteManager.instance }
    ...
    val LocalEditorSnackbarHostState =
        compositionLocalOf<SnackbarHostState> { noLocalProvidedFor("LocalEditorSnackbarHostState") }
}
...
class EditorActivity : BaseComposeActivity() {
    ...
    private fun onCreate() {
        ...
        CommandPaletteManager.instance.addCommand(
            ...
            newCommand("Settings", "Ctrl+,") { open(SettingsActivity::class.java) },
            newCommand("Manage Plugins", "Ctrl+P") { open(PluginsActivity::class.java) },
            ...
            newCommand(getString(strings.generate_code), "Alt+I") { compositionContext ->
                ...
                val composeView =
                    ComposeView(this@EditorActivity).apply {
        ...
        ProvideEditorCompositionLocals {
            ...
            ModalNavigationDrawer(
                ...
                Scaffold(
                    ...
                    topBar = { EditorTopBar(editorViewModel = editorViewModel) },
    ...
    private fun clearCache(): Boolean {
        return FileUtils.deleteAllInDir(cacheDir).also { Log.i(TAG, "Cache cleared 😊") }
    }
            ...
            when (event) {
                Lifecycle.Event.ON_CREATE -> {
                    ...
                    lifecycleScope.launch {
                        ...
                        runCatching { PluginLoader.loadPlugins(this@EditorActivity) }
                            .onSuccess { plugins ->
                                val pluginContext =
                                    PluginContextImpl(
                                        this@EditorActivity,
    ...
    val currentEditor
        get() = editorViewModel.getSelectedEditor()

    val selectedFileIndex
        get() = editorViewModel.uiState.value.selectedFileIndex

    val canEditorHandleCurrentKeyBinding
        get() = editorViewModel.canEditorHandleCurrentKeyBinding.value

    val editorForFile: (File) -> CodeEditorView? = { file ->
        editorViewModel.getEditorForFile(file)
    }

    @JvmField val openFile: (File) -> Unit = { file -> editorViewModel.addFile(file) }

    @JvmField val closeFile: (Int) -> Unit = { index -> editorViewModel.closeFile(index) }

    @JvmField val closeAll = { editorViewModel.closeAll() }

    @JvmField val closeOthers: (Int) -> Unit = { index -> editorViewModel.closeOthers(index) }

    @JvmField val saveAll = { lifecycleScope.launch { editorViewModel.saveAll() } }

    val openedFolder = { fileExplorerViewModel.openedFolder.value }

    @JvmOverloads
    fun saveFile(codeEditorView: CodeEditorView? = null) {
        lifecycleScope.launch { editorViewModel.saveFile(codeEditorView) }
    }
}
