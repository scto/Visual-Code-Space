...
abstract class BaseComposeActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            VCSpaceTheme {
                ...
                val darkTheme by
                    remember(followSystemTheme, localDarkTheme, systemDarkTheme) {
    ...
    @Composable protected abstract fun MainScreen()
    ...
    private fun setupKotlinStdlib() {
        ...
        if (!File(kotlinStdlib).exists()) {
            assets.open("kotlin-stdlib-2.0.20.jar").use { input ->
                File(kotlinStdlib).outputStream().use { output -> input.copyTo(output) }
            }
        }
...
fun ObserveLifecycleEvents(onStateChanged: (Lifecycle.Event) -> Unit) {
    ...
    DisposableEffect(lifecycleOwner) {
        val observer = LifecycleEventObserver { _, event -> currentOnStateChanged(event) }
