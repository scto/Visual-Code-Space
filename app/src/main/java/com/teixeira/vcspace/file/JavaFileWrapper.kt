...
internal data class JavaFileWrapper(private val raw: java.io.File) : File {
    ...
    override fun uri(context: Context): Uri =
        FileProvider.getUriForFile(context, "${context.packageName}.provider", raw)
    ...
    ): Boolean = withContext(ioDispatcher) { FileIOUtils.writeFileFromString(raw, content) }
}
