...
class GitViewModel : ViewModel() {
    ...
    suspend fun loadRepoName(onFailure: OnFailureListener = {}) {
        ...
        withContext(Dispatchers.IO) {
            runCatching { git.getRepositoryName() }
                .onSuccess { name ->
    ...
    val unpushedCommits
        get() = _unpushedCommits.asStateFlow()

    suspend fun loadUnpushedCommits(onFailure: OnFailureListener = {}) {
        withContext(Dispatchers.IO) {
            runCatching { git.getUnpushedCommits() }
                .onSuccess { commits -> _unpushedCommits.update { commits } }
                .onFailure { throwable -> withContext(Dispatchers.Main) { onFailure(throwable) } }
        }
    }

    private val _changeStats = MutableStateFlow(GitChangeStats())
    val changeStats
        get() = _changeStats.asStateFlow()

    suspend fun loadChangeStats(onFailure: OnFailureListener = {}) {
        _changeStats.update { it.copy(isLoading = true) }
        _gitStatus.update { GitActionStatus.Loading(null, "Loading changes stats") }

        withContext(Dispatchers.IO) {
            runCatching { git.getUncommittedChangesStats() }
                .onSuccess { stats ->
                    _changeStats.update { it.copy(isLoading = false, changeStats = stats) }
                    _gitStatus.update { GitActionStatus.Success("Loading changes stats success") }
                }
                .onFailure { throwable ->
                    _changeStats.update { it.copy(isLoading = false) }
                    _gitStatus.update { GitActionStatus.Failure(throwable) }
    ...
    suspend fun fetch(onFailure: OnFailureListener = {}) {
        ...
        withContext(Dispatchers.IO) {
            runCatching {
                ...
                .onSuccess { _gitStatus.update { GitActionStatus.Success("Fetch success") } }
                .onFailure { throwable ->
                    _gitStatus.update { GitActionStatus.Failure(throwable) }
    ...
    suspend fun pull(onFailure: OnFailureListener = {}) {
        ...
        withContext(Dispatchers.IO) {
            runCatching {
                ...
                .onSuccess { _gitStatus.update { GitActionStatus.Success("Pull changes success") } }
                .onFailure { throwable ->
                    _gitStatus.update { GitActionStatus.Failure(throwable) }
    ...
    suspend fun refresh(onFailure: OnFailureListener = {}) {
        ...
        withContext(Dispatchers.IO) {
            runCatching {
                ...
                .onSuccess { _gitStatus.update { GitActionStatus.Success("Refreshing success") } }
                .onFailure { throwable ->
                    _gitStatus.update { GitActionStatus.Failure(throwable) }
    ...
    suspend fun push(onFailure: OnFailureListener = {}) {
        ...
        withContext(Dispatchers.IO) {
            runCatching {
                ...
                .onSuccess { _gitStatus.update { GitActionStatus.Success("Successfully pushed") } }
                .onFailure { throwable ->
...
data class GitChanges(val isLoading: Boolean = false, val fileChanges: List<String> = emptyList())

data class GitChangeStats(val isLoading: Boolean = false, val changeStats: ChangeStats? = null)
