...
class JavaIncrementalAnalyzeManager :
            ...
            if (state.state!!.hasBraces || checkForIdentifiers) {
                ...
                for (i1 in state.tokens.indices) {
                    ...
                    if (type > 0) {
                        if (isStart(token)) {
                            bracketsStack.push(
                                IntPair.pack(type, text.getCharIndex(i, tokenRecord.offset))
                            )
                        } else {
    ...
    override fun generateSpansForLine(
        lineResult: LineTokenizeResult<State, HighlightToken?>
    ): MutableList<Span> {
        ...
        for (i in tokens.indices) {
            ...
            when (token) {
                ...
                Tokens.IDENTIFIER -> {
                    ...
                    if (
                        token == Tokens.LBRACK ||
                            (token == Tokens.RBRACK && previous == Tokens.LBRACK)
                    ) {
