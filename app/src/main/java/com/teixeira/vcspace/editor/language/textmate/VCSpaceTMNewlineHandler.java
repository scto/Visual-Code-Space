...
public class VCSpaceTMNewlineHandler implements NewlineHandler {
  ...
  public NewlineHandleResult handleNewline(
      @NonNull Content text, @NonNull CharPosition position, @Nullable Styles style, int tabSize) {
    ...
    switch (enterAction.indentAction) {
      ...
      case Indent:
        {
          ...
          // offset value is not needed because the editor ignores the position of invisible
          // characters when moving the cursor
      ...
      case Outdent:
        final var indentation =
            TextUtils.getIndentationFromWhitespace(
                enterAction.indentation, language.tabSize, language.useTab());
        final var outdentedText =
  ...
  protected Pair<String, String> getIndentForEnter(Content text, CharPosition position) {
    ...
    var beforeEnterIndent =
        TextUtils.getLeadingWhitespace(beforeEnterText, 0, beforeEnterText.length());

    var afterEnterAction =
        getInheritIndentForLine(
            new VCSpaceTMNewlineHandler.WrapperContentImp(text, position.line, beforeEnterText),
            true,
            position.line + 1);
    ...
    if (afterEnterAction.action == EnterAction.IndentAction.Indent) {
      ...
      // var invisibleColumn = TextUtils.invisibleColumnFromColumn(text, position.line,
      // position.column, tabSpaces);
    ...
    if (indentRulesSupport.shouldDecrease(afterEnterText)) {
      ...
      afterEnterIndent =
          beforeEnterIndent.substring(
              0,
              Math.max(
                  Math.max(0, beforeEnterIndent.length() - 1),
  ...
  /**
   * Get inherited indentation from above lines. 1. Find the nearest preceding line which doesn't
   * match unIndentedLinePattern. 2. If this line matches indentNextLinePattern or
   * increaseIndentPattern, it means that the indent level of `lineNumber` should be 1 greater than
   * this line. 3. If this line doesn't match any indent rules a. check whether the line above it
   * matches indentNextLinePattern b. If not, the indent level of this line is the result c. If so,
   * it means the indent of this line is *temporary*, go upward utill we find a line whose indent is
   * not temporary (the same workflow a -> b -> c). 4. Otherwise, we fail to get an inherited indent
   * from aboves. Return null and we should not touch the indent of `lineNumber`
   *
   * <p>This function only return the inherited indent based on above lines, it doesn't check
   * whether current line should decrease or not.
   */
  @Nullable
  private VCSpaceTMNewlineHandler.InheritIndentResult getInheritIndentForLine(
      VCSpaceTMNewlineHandler.WrapperContent wrapperContent,
      boolean honorIntentialIndent,
      int line) {
    ...
    if (indentRulesSupport.shouldIncrease(precedingUnIgnoredLineContent)
      ...
      return new VCSpaceTMNewlineHandler.InheritIndentResult(
          TextUtils.getLeadingWhitespace(
              precedingUnIgnoredLineContent, 0, precedingUnIgnoredLineContent.length()),
      ...
      if (precedingUnIgnoredLine == 0) {
        return new VCSpaceTMNewlineHandler.InheritIndentResult(
            TextUtils.getLeadingWhitespace(wrapperContent.getLineContent(precedingUnIgnoredLine)),
            null,
            precedingUnIgnoredLine);
      }
      ...
      var previousLineIndentMetadata =
          indentRulesSupport.getIndentMetadata(wrapperContent.getLineContent(previousLine));

      // 	if (!(previousLineIndentMetadata & (IndentConsts.INCREASE_MASK |
      // IndentConsts.DECREASE_MASK)) &&
      //			(previousLineIndentMetadata & IndentConsts.INDENT_NEXTLINE_MASK)) {
      if (((previousLineIndentMetadata
                  & (IndentRulesSupport.IndentConsts.INCREASE_MASK
                      | IndentRulesSupport.IndentConsts.DECREASE_MASK))
              == 0)
          && (previousLineIndentMetadata & IndentRulesSupport.IndentConsts.INDENT_NEXTLINE_MASK)
              == 0
        ...
        return new VCSpaceTMNewlineHandler.InheritIndentResult(
            TextUtils.getLeadingWhitespace(wrapperContent.getLineContent(stopLine + 1)),
            null,
            stopLine + 1);
      }
      ...
      for (var i = precedingUnIgnoredLine; i > 0; i--) {
        ...
        if (indentRulesSupport.shouldIncrease(lineContent)) {
          return new VCSpaceTMNewlineHandler.InheritIndentResult(
              TextUtils.getLeadingWhitespace(lineContent), EnterAction.IndentAction.Indent, i);
          ...
          return new VCSpaceTMNewlineHandler.InheritIndentResult(
              TextUtils.getLeadingWhitespace(lineContent), null, i);
        }
      }

      return new VCSpaceTMNewlineHandler.InheritIndentResult(
          TextUtils.getLeadingWhitespace(wrapperContent.getLineContent(1)), null, 1);
    }
  }

  /**
   * Get nearest preceding line which doesn't match unIndentPattern or contains all whitespace.
   * Result: -1: run into the boundary of embedded languages 0: every line above are invalid else:
   * nearest preceding line of the same language
   */
  public int getPrecedingValidLine(VCSpaceTMNewlineHandler.WrapperContent content, int lineNumber) {
    ...
    if (lineNumber > 0) {
      ...
      for (lastLineNumber = lineNumber - 1; lastLineNumber >= 0; lastLineNumber--) {
        ...
        if (indentRulesSupport.shouldIgnore(
                text) /*|| precedingValidPattern.matcher(text).matches()*/
            || text.isEmpty()) {
          continue;
        }
  ...
  /**
   * @see <a
   *     href="https://github.com/microsoft/vscode/blob/main/src/vs/editor/common/languages/enterAction.ts">
  ...
  public CompleteEnterAction getEnterAction(final Content content, final CharPosition position) {
    ...
    var beforeEnterText =
        scopedLineText.substring(0, position.column /*- 0*/ /*scopedLineTokens.firstCharOffset*/);
    ...
    // afterEnterText = scopedLineText.substr(range.startColumn - 1 -
