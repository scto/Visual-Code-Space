...
open class JavaTextTokenizer(src: CharSequence?) {
    ...
    protected fun scanIdentifier(ch: Char): Tokens {
        ...
        while (
            offset + tokenLength < bufferLen &&
                isIdentifierPart(charAt(offset + tokenLength).also { ch = it })
        ) {
            tokenLength++
            n = n?.map?.get(ch)
        }
        return if (n == null) Tokens.IDENTIFIER
        else ((if (n.token == null) Tokens.IDENTIFIER else n.token)!!)
    }
    ...
    companion object {
        ...
        protected fun doStaticInit() {
            ...
            for (i in sKeywords.indices) {
                tree!!.put(sKeywords[i], sTokens[i])
            }
        }
    }
