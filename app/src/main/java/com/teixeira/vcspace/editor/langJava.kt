 ...
 * @author Akash Yadav
 */
class TsLanguageJava(languageSpec: JavaLanguageSpec, tab: Boolean = false) :
    TsLanguage(languageSpec, tab, { buildTheme() }) {

    override val analyzer: TsAnalyzeManager by lazy { TsJavaAnalyzeManager(languageSpec, tsTheme) }
}

class TsJavaAnalyzeManager(languageSpec: TsLanguageSpec, theme: TsTheme) :
...
class TsJavaSpanFactory(
    ...
    override fun createSpans(capture: TSQueryCapture, column: Int, spanStyle: Long): List<Span> {
        ...
        val (start, end) =
            content.indexer.run {
                getCharPosition(capture.node.startByte / 2) to
                    getCharPosition(capture.node.endByte / 2)
            }
...
fun TsThemeBuilder.buildTheme() {
    ...
    makeStyle(IDENTIFIER_VAR) applyTo arrayOf("variable.builtin", "variable", "constant")
    makeStyle(IDENTIFIER_NAME) applyTo arrayOf("type.builtin", "type", "attribute")
    makeStyle(FUNCTION_NAME) applyTo
        arrayOf("function.method", "function.builtin", "variable.field")
